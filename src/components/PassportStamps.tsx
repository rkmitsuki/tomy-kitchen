...
export default function PassportStamps() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (progress) => {
    const next = Math.min(stamps.length - 1, Math.max(0, Math.floor(progress * stamps.length)));
    setActiveIndex(next);
  });
  ...
  return (
    <div
      ref={containerRef}
      className="relative overflow-hidden rounded-[2rem] border-2 border-dashed border-white/15 bg-[var(--kitchen-night)] bg-[repeating-linear-gradient(135deg,rgba(255,255,255,0.025)_0px,rgba(255,255,255,0.025)_2px,transparent_2px,transparent_14px)] p-8 sm:p-12"
    >
